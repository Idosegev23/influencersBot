...
הכלי הקריאייטיבי הכי מורכב בצוות לידרס הפך מ-"שבוע של עבודה" ל-"שעה של עריכה". מאפשר לצוות לקחת יותר לקוחות, לייצר יותר הצעות, ולהשקיע את הזמן בעידון ולא בבנייה מ-0.`,
  ...
  {
    num: 19,
    source_id: 'ldrs-conf-2026-bestie-platform',
    title: 'בסטי — פלטפורמת ה-AI Agent שלידרס בונה ללקוחות',
    metadata: { topic: 'ai_products', type: 'product', product: 'bestie' },
    text: `# בסטי (Bestie) — פלטפורמת AI Agent של לידרס
# "Bestie יודע הכל" — מה שאיתמר אמר בסיום ההרצאה

## מה זה
בסטי הוא **המוצר** שעליו לידרס דיברו בסוף ההרצאה (ה-QR Code שמופיע במצגת מוביל לכאן). זוהי פלטפורמת AI agent רב-לקוחית שלידרס בונים מאפס: סוכן AI מותאם-מותג שיודע הכל על המוצרים, השירותים, הקייסים והמומחיות של הלקוח, מנהל שיחה טבעית ומוביל לפעולה (הזמנה, פגישה, רכישה).

## למי זה מתאים
- מותגים שרוצים סוכן AI חכם שמדבר בשפת המותג ומוצמד לקטלוג המוצרים שלהם
- חברות שירות שצריכות סוכן שמסביר את ההצעה ומסנן לידים איכותיים לפני פגישה
- עסקים שיש להם הרבה תוכן (אינסטגרם, תמלולי וידאו, אתר) ורוצים שהקהל שלהם יוכל לדבר עם זה כמו עם בנאדם
- כנסים, אירועים, פעולות שיווק נקודתיות — סוכן ייעודי לאירוע

## איך זה עובד
1. **ingest** — לידרס סורקים את כל התוכן של הלקוח: אתר, אינסטגרם, פוסטים, תמלולי ריילים, סטוריז, האב מסמכים, FAQ, קייסים. הכל הופך ל-RAG עם embeddings.
2. **persona** — בניית פרסונה דיגיטלית: הקול של המותג, נושאי ליבה, גבולות שיחה, מדיניות תשובה, ביטויים אופייניים. הכל מבוסס על תוכן אמיתי, לא דמיוני.
3. **chat surface** — חוויית שיחה ב-3 ערוצים: דף chat ייעודי (כמו זה!), widget מוטמע באתר הלקוח, או DM באינסטגרם.
4. **business logic** — אינטגרציה לתהליכי הלקוח: בדיקת קופונים, ניהול הזמנות, תיאום פגישות, יצירת לידים, דיווח לכלי CRM, webhook ל-Make.

## טכנולוגיה
- **Multi-model**: GPT-5.4 ראשי + GPT-5.2 fallback + Gemini Flash לתמלול ו-OCR + Claude לעיבוד מסמכים
- **Hybrid retrieval**: vector embeddings (text-embedding-3-large, 2000d) + BM25 + grounded facts → 4-stage retrieval
- **Real-time streaming**: התשובה מתחילה להיכתב מיד, לא ממתינים
- **Memory**: שיחות נשמרות, summary rolling, conversation insights
- **Multi-tenant**: כל לקוח חשבון עצמאי עם persona, knowledge, theme, ו-config משלו

## מקרי שימוש שיש בפלטפורמה
המוצר רץ כיום על מספר קליינטים של לידרס. בלי לחשוף שמות פרטיים, השימושים כוללים:
- **Brand widget**: e-commerce שמשלב את בסטי באתר עם 100+ מוצרים, המלצות AI, חיפוש שיחתי
- **Creator chat**: יוצרי תוכן שעובדים עם לידרס מקבלים סוכן אישי שעוקבים שלהם יכולים לדבר איתו
- **Service provider chat**: סוכנויות (כמו לידרס עצמה!) שצריכות סוכן B2B שמסביר את השירות ומקרב ללקוחות חדשים — בדיוק מה שאתם רואים פה עכשיו
- **Conference companion**: סוכן ייעודי לאירוע (כמו הכנס הזה) שמתחבר לתוכן ההרצאה ומפנה לפעולה

## בסטי + הטמעה ארגונית
בסטי הוא דוגמה מעשית ל-"AI ארגוני, לא AI פרטי" שאיתמר דיבר עליו בהרצאה. זה לא צ'אטבוט גנרי — זה סוכן ארגוני שמחובר למערכות, לתוכן, לפלואו העסקי, ולאנשים מאחורי המותג.

## ההבטחה
תוך כמה שבועות, מותג מקבל סוכן AI שמדבר בקול שלו, יודע הכל על המוצרים שלו, ומוביל את הקהל לפעולות עסקיות אמיתיות. בלי טמפלייטים, בלי "ChatGPT עם prompt חכם" — מערכת שלמה שלידרס בונה.`,
  },
  {
    num: 20,
    source_id: 'ldrs-conf-2026-full-automations',
    title: 'אוטומציות שלמות — הגישה של לידרס לתהליכים אוטומטיים מקצה לקצה',
    metadata: { topic: 'ai_products', type: 'methodology', product: 'ai_automations' },
    text: `# אוטומציות שלמות — תהליכים מקצה לקצה
# הגישה של לידרס ל-Workflow Automation עם AI

## העקרון
"אוטומציה" אצל לידרס לא אומרת trigger בודד ב-Make. אומרת: **תהליך עסקי שלם** שמתחיל באירוע אחד (בריף לקוח חדש, ליד חם, פגישה שהסתיימה) ומסיים בפעולה אחרונה (הצעה ביד הלקוח, לידי בדשבורד, מצגת ב-Drive). הטכנולוגיה היא Make + AI APIs + מערכות הלקוח. הערך הוא **חיסכון של ימי עבודה** על תהליכים שלא צריכים אדם לבצע.

## דוגמה 1: התהליך הפנימי של לידרס (Leaders Platform end-to-end)
זה לא תיאוריה. זה מה שלידרס עצמה משתמשת בו יום-יום. ה-pipeline:

1. **בריף לקוח דיגיטלי** — לקוח מקבל לינק, ממלא טופס רב-שלבי בלי חשבון/סיסמה. localStorage שומר אוטומטית.
2. **התראה לצוות** — ברגע שהבריף מוגש, מערכת מתחילה לעקוב (sent → opened → completed). תזכורות אוטומטיות אחרי 7 ימי עסקים אם לא הושלם.
3. **פגישת התנעה (Inner Meeting)** — אחרי קבלת בריף, צוות מזמן פגישת kickoff עם טופס שיתופי בזמן אמת. כולם ממלאים יחד, אין צורך ב"מי כותב סיכום".
4. **מחולל הצעת מחיר** — מהבריף + ה-kickoff, עובד בונה הצעת מחיר אינטראקטיבית. בחירת שירותים, תקציב, KPIs, פלטפורמות. יצוא PPTX/PDF.
5. **מצגת קריאייטיבית AI** — בלחיצה אחת, סוכן Gemini בונה מצגת 11 שקפים מהבריף: מחקר מותג, ניתוח מתחרים, אסטרטגיית משפיענים, לוח תוכן, ויזואלים. הכל מוכן לשליחה ללקוח.
6. **Cron יומי 08:00 UTC** — בודק מסמכים פתוחים, פגישות שלא הסתיימו, deadlines קרובים. שולח תזכורות דרך Gmail או Make.com webhook.
7. **Drive auto-upload** — כל מסמך שהושלם נשמר אוטומטית בתיקיית הפרויקט בגוגל דרייב.

**התוצאה**: מ-יום עבודה שלם על הצעת מחיר אחת — לשעה ביום. Ratio 80% AI, 20% אנושי. **זה הציטוט של איתמר מההרצאה** — לא הבטחה, מספר ראלי.

## דוגמה 2: לידים מהכנס הזה (האוטומציה הזאת רצה עכשיו!)
1. **QR Code** במצגת הסיום של איתמר → URL של הצ'אט עם param ?source=conf
2. **הסטארטרים** משתנים אוטומטית לשאלות ממוקדות AI (סיכום ההרצאה, איך מתחילים)
3. **שיחה** — הבוט עונה ממסד הידע של ההרצאה + 5 שלבים + 4 עקרונות verbatim
4. **טופס פגישה** — ברגע שהמשתמש לוחץ "קבעו פגישה", מתמלא טופס qualifying (פרטי קשר + תחום עניין + שימוש ב-AI היום + טווח זמן + כאב)
5. **POST ל-/api/leads/conference** — Next.js route שומר ב-DB
6. **Make webhook** — fire-and-forget POST עם payload של 41 שדות
7. **Make scenario** — מנתב ל-Sheets (ללוג) + מייל לרועי ("ליד חדש מהכנס") + slack notification
8. **Roi קורא**, מתאם פגישה ב-Calendly, ה-Calendly שולח אישור — האוטומציה ממשיכה

**מהזמן שמישהו סיים שיחה ועד שרועי קיבל מייל = שניות**.

## דוגמה 3: סקאן יומי של חשבונות לקוחות
לכל לקוח של בסטי יש cron שרץ:
- **01:00 UTC** — סקאן Instagram מלא: פוסטים חדשים, ריילים, סטוריז, היילייטס, תגובות. תמלול אוטומטי של וידאו (Gemini Flash). הפיכה ל-RAG chunks. הבוט יודע מה חדש למחרת בבוקר.
- **02:00 UTC** — עדכון פרסונה: מהפוסטים החדשים, GPT-5.4 מעדכן את ה-knowledge_map ואת ה-voice_rules. הקול של הלקוח לומד את עצמו.
- **06:00 UTC** — ניתוח שיחות מהיום הקודם: מה אנשים שאלו, מה הבוט לא ידע, מה הצליח להוביל לרכישה. תובנות לדשבורד.

**זה אומר**: הבוט תמיד מעודכן, בלי שמישהו מהצוות נוגע בזה.

## למה זה שונה מ-"Make scenario רגיל"
- **שילוב AI אמיתי**: הסוכנים בכל שלב הם LLM (GPT, Claude, Gemini), לא if/else.
- **שילוב מערכות**: לא רק 2 כלים. CRM + Drive + Gmail + Make + Supabase + APIs פנימיים.
- **משוב אנושי**: לכל אוטומציה יש שלב human-in-the-loop ברגעים הקריטיים. AI מנסח, אדם מאשר. 80/20.
- **תפעול לאורך זמן**: מערכת חיה, לא scenario חד-פעמי. מעקב, תזכורות, alerts על דברים שנתקעו.

## מה לידרס בונה ללקוחות
לקוחות שצריכים אוטומציה שלמה (לא רק "תרגם מ-Slack ל-WhatsApp"):
- **Onboarding flow** — מהרגע שלקוח חתם, עד שהוא מקבל ערך
- **Lead-to-meeting pipeline** — האב + AI scoring + automatic routing
- **Content production** — מ-brief ועד פוסט מוכן לפרסום
- **Customer support** — סוכן AI שעונה ב-90% מהמקרים, מסלים ל-15% הקשים
- **Reporting & insights** — דשבורדים שמתעדכנים בזמן אמת, AI מנתח טרנדים

## ההמלצה
אם אתם חושבים על "אוטומציה" רק כ-trigger בודד — זה זמן לחשוב מחדש. תהליך אוטומטי שלם חוסך עשרות שעות בשבוע. לידרס בונים את זה, מקצה לקצה, עם ההבנה של ה-DNA של העסק שלכם. בואו נדבר.`,
  },
  {
    num: 18,
