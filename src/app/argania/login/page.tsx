...
// Argania account. Username drives the influencer-cookie auth + dashboard route;
// the id is only used to pull branding from the public widget config so this
// page stays in sync with the account instead of hardcoding logo/colors.
const ACCOUNT_USERNAME = 'argania_group';
const ACCOUNT_ID = 'c68ef2bd-f294-4c8c-83dc-abd5f9cbf6d1';
const SUPPORT_PATH = `/influencer/${ACCOUNT_USERNAME}/support`;
const FALLBACK_COLOR = '#2d5016';
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!cancelled && data) {
          setBrand({
            name: data.brandName || 'ARGANIA GROUP',
