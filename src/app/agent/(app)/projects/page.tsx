...
export default function ProjectsPage() {
  ...
  if (!data?.tables) return <div dir="rtl" className="text-sm text-red-600">שגיאה בטעינת הפרויקטים</div>;

  const t = data.tables;
  const single = [...(t.single_signed || []), ...(t.single_not_signed || [])];
  const multi = [...(t.multi_signed || []), ...(t.multi_not_signed || [])];
  const signedCount = (t.single_signed?.length || 0) + (t.multi_signed?.length || 0);
  const total = single.length + multi.length;

  return (
    <div dir="rtl">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-[color:var(--ink-900)]">פרויקטים</h1>
        <p className="text-sm text-[color:var(--ink-500)] mt-1">כל הפרויקטים לפי סוג וסטטוס</p>
      </div>

      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
        <Kpi label="סה״כ פרויקטים" value={String(total)} />
        <Kpi label="נחתמו" value={String(signedCount)} />
        <Kpi label="חד-חודשיים" value={String(single.length)} />
        <Kpi label="רב-חודשיים" value={String(multi.length)} />
      </div>
