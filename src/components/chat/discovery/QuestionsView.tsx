    ...
    return (
      <div className="p-4">
        <div className="flex items-center gap-2 mb-4">
          <button onClick={onBack} className="w-9 h-9 rounded-full flex items-center justify-center discovery-glass-card">
            <ChevronRight className="w-5 h-5" style={{ color: '#888' }} />
          </button>
          <div className="h-5 w-48 bg-white/40 rounded animate-pulse" />
        </div>
        {Array.from({ length: 4 }).map((_, i) => (
          <div key={i} className="h-16 bg-white/30 rounded-[20px] mb-2.5 animate-pulse" />
        ))}
      </div>
    );
  }
    ...
    >
      ...
      {data && data.previousAnswers.length > 0 && (
        <div className="px-0 mb-4">
          <h3 className="text-[14px] font-bold mb-2" style={{ color: '#8b5cf6' }}>
            תשובות מהשבוע שעבר
          </h3>
          <div className="space-y-2.5">
            {data.previousAnswers.map((q) => (
              <motion.div
                ...
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                className="discovery-glass-card p-3.5"
              >
                <div className="flex items-start gap-2">
                  ...
                  <div>
                    ...
                    {q.answerText && (
                      <p className="text-[13px] mt-1 leading-relaxed" style={{ color: '#666' }}>
                        {q.answerText}
                      </p>
                    )}
      ...
      <div className="px-0 mb-5">
        <h3 className="text-[14px] font-bold mb-2" style={{ color: '#1a1a2e' }}>
          שאלו שאלה חדשה
        </h3>
        <div className="flex gap-2">
          <input
            ...
            className="discovery-glass-input flex-1"
          ...
          <button
            ...
            style={{ background: 'linear-gradient(135deg, #8b5cf6 0%, #d946ef 50%, #f472b6 100%)' }}
          >
      ...
      {data && data.currentWeek.length > 0 && (
        <div className="px-0 pb-6">
          <div className="flex items-center justify-between mb-2">
            <h3 className="text-[14px] font-bold" style={{ color: '#1a1a2e' }}>
              השאלות של השבוע
            </h3>
            {data.weekLabel && (
      ...
      {data && data.currentWeek.length === 0 && (
        <div className="px-0 pb-6 text-center">
...
function QuestionItem({
  ...
  return (
    <motion.div
      ...
      className="discovery-glass-card flex items-center gap-3 p-3.5"
    >
      {/* Vote button */}
      <button
      ...
      >
        <ThumbsUp
          ...
          style={{ color: question.hasVoted ? '#8b5cf6' : '#999' }}
          fill={question.hasVoted ? '#8b5cf6' : 'none'}
        />
        <span className="text-[11px] font-bold" style={{ color: question.hasVoted ? '#8b5cf6' : '#999' }}>
          {question.voteCount}
        </span>
      </button>

      {/* Question text */}
      <p className="flex-1 text-[14px] leading-tight" style={{ color: '#1a1a2e' }} dir="rtl">
