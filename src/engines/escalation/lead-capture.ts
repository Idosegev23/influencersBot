...
export function leadDiggingInstruction(brandName: string): string {
  return (
    ...
    `פגישות: אין לך גישה ליומן ואתה לא יכול לקבוע כלום. אל תאשר מועד, שעה, תאריך, לינק לזום או הזמנה ליומן, ` +
    `ואל תגיד "קבענו" או "נשלח לך לינק". מותר ומומלץ לאסוף זמינות — לשאול מתי נוח ובאיזה פורמט — ולומר במפורש ` +
    `שנציג/ה מהצוות יחזרו לתאם ולאשר סופית.]`
  );
}
