...
export abstract class BaseArchetype {
  ...
  protected async generateAIResponse(
    ...
    try {
      ...
      const systemPrompt = `אתה ${influencerName}, משפיענית שמנהלת שיחה אישית וחמה עם הקהל שלה — כמו חברה טובה, לא כמו מכונת חיפוש.
...
• **אחרי כל תשובה**: הציע/י בקצרה המשך טבעי — "רוצה טיפ להגשה?" / "יש לי עוד גרסה בלי שמנת, מעניין?" — אבל רק שאלה אחת, לא רשימה.
