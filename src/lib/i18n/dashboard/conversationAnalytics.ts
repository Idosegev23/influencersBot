// Conversation analytics page (weekly retro report).
export const conversationAnalytics = {
  he: {
    pageTitle: 'ניתוח שיחות',
    linkFromAnalytics: 'ניתוח שיחות מפורט',
    backToAnalytics: 'חזרה לאנליטיקס',
    coverage: '{total} שיחות בטווח · {classified}% סווגו · {product}% מהתלונות שויכו למוצר',
    coverageHint: 'האחוזים כאן קיימים כדי שלא תסיקו מסקנה ממדגם חלקי',
    kpiTotal: 'סה"כ פניות',
    kpiComplaints: 'תלונות',
    kpiResolved: 'נפתרו ע"י הבוט',
    kpiEscalated: 'הוסלמו',
    kpiNegative: 'סנטימנט שלילי',
    vsPrevious: 'מול התקופה הקודמת',
    sectionInsights: 'תובנות התקופה',
    insightsEmpty: 'אין עדיין תובנות לתקופה הזו',
    insightShowSessions: 'הצג את {n} השיחות',
    sectionWhatTheyTalkedAbout: 'על מה דיברו',
    sectionInquiryTypes: 'סוגי פנייה',
    sectionTopics: 'נושאים מובילים',
    topicNew: 'חדש',
    sectionComplaints: 'תלונות',
    complaintsByKind: 'לפי סוג תלונה',
    complaintsByProduct: 'תלונה מול מוצר',
    complaintsByCategory: 'תלונה מול קטגוריה',
    sectionSeries: 'סדרות',
    seriesByRate: 'לפי שיעור תלונה',
    seriesByMentions: 'הכי מדוברות',
    colSeries: 'סדרה',
    seriesHint: 'לקוחות נוקבים בשם סדרה הרבה יותר מאשר במוצר בודד, ולכן רוב השיוך נוחת כאן',
    lowSample: 'מדגם קטן',
    sectionProducts: 'מוצרים',
    productsByRate: 'לפי שיעור תלונה',
    productsByMentions: 'הכי מדוברים',
    productsRateHint: 'שיעור תלונה = תלונות חלקי אזכורים. מוצר נמכר צובר תלונות מעצם היותו נמכר.',
    colProduct: 'מוצר',
    colMentions: 'אזכורים',
    colComplaints: 'תלונות',
    colComplaintRate: 'שיעור תלונה',
    colCount: 'כמות',
    colCategory: 'קטגוריה',
    sectionChannels: 'ערוצים',
    channelNotConnected: 'לא מחובר',
    channelWeb: 'ווידג׳ט ועמוד צ׳אט',
    channelWhatsapp: 'וואטסאפ',
    channelInstagram: 'אינסטגרם',
    sectionWatchKeywords: 'מילות מעקב',
    watchHint: 'המילים שביקשתם לעקוב אחריהן, נספרות בטקסט השיחות עצמו. הפיצול קיים כי מילה אחת יכולה לחיות בשני עולמות — "פגום" מתאר גם מוצר שהגיע פגום וגם שיער פגום.',
    watchColTerm: 'מילה',
    watchColSessions: 'שיחות',
    watchColComplaints: 'מהן תלונות',
    watchColOther: 'לא תלונות',
    watchUnused: 'לא הופיעה בשיחות בטווח הזה',
    watchColUnclassified: 'טרם סווגו',
    watchNotYetKnown: 'טרם סווג',
    watchPendingHint: 'חלק מהשיחות עדיין לא סווגו, ולכן הפיצול לתלונות חלקי. "טרם סווג" אינו "אין תלונות".',
    sectionKeywords: 'מילות מפתח שהמערכת זיהתה',
    sectionTable: 'פירוט השיחות',
    colDate: 'תאריך',
    colChannel: 'ערוץ',
    colInquiryType: 'סוג פנייה',
    colTopic: 'נושא',
    colComplaint: 'תלונה',
    colSummary: 'תקציר',
    export: 'ייצוא',
    exportAggregates: 'ייצוא נתונים מצרפיים',
    exportWithMessages: 'ייצוא כולל תוכן שיחות',
    exportWithMessagesWarning: 'הקובץ יכיל תוכן פניות של לקוחות. להמשיך?',
    filterAll: 'הכל',
    filterComplaintsOnly: 'תלונות בלבד',
    filterClear: 'נקה סינון',
    loading: 'טוען…',
    empty: 'אין נתונים בטווח שנבחר',
    prev: 'הקודם',
    next: 'הבא',
    yes: 'כן',
    no: 'לא',
  },
  en: {
    pageTitle: 'Conversation analysis',
    linkFromAnalytics: 'Detailed conversation analysis',
    backToAnalytics: 'Back to analytics',
    coverage: '{total} conversations in range · {classified}% classified · {product}% of complaints matched to a product',
    coverageHint: 'These percentages exist so a partial sample is not mistaken for a complete one',
    kpiTotal: 'Total inquiries',
    kpiComplaints: 'Complaints',
    kpiResolved: 'Resolved by bot',
    kpiEscalated: 'Escalated',
    kpiNegative: 'Negative sentiment',
    vsPrevious: 'vs previous period',
    sectionInsights: 'Insights',
    insightsEmpty: 'No insights for this period yet',
    insightShowSessions: 'Show the {n} conversations',
    sectionWhatTheyTalkedAbout: 'What they talked about',
    sectionInquiryTypes: 'Inquiry types',
    sectionTopics: 'Top topics',
    topicNew: 'new',
    sectionComplaints: 'Complaints',
    complaintsByKind: 'By complaint kind',
    complaintsByProduct: 'Complaint by product',
    complaintsByCategory: 'Complaint by category',
    sectionSeries: 'Product lines',
    seriesByRate: 'By complaint rate',
    seriesByMentions: 'Most discussed',
    colSeries: 'Line',
    seriesHint: 'Customers name a product line far more often than a single SKU, so most attribution lands here',
    lowSample: 'small sample',
    sectionProducts: 'Products',
    productsByRate: 'By complaint rate',
    productsByMentions: 'Most discussed',
    productsRateHint: 'Complaint rate = complaints divided by mentions. A bestseller accrues complaints by virtue of selling.',
    colProduct: 'Product',
    colMentions: 'Mentions',
    colComplaints: 'Complaints',
    colComplaintRate: 'Complaint rate',
    colCount: 'Count',
    colCategory: 'Category',
    sectionChannels: 'Channels',
    channelNotConnected: 'not connected',
    channelWeb: 'Widget & chat page',
    channelWhatsapp: 'WhatsApp',
    channelInstagram: 'Instagram',
    sectionWatchKeywords: 'Watched keywords',
    watchHint: 'The terms you asked us to track, counted in the conversation text itself. The split exists because one word can live in two worlds — "damaged" describes both a product that arrived damaged and damaged hair.',
    watchColTerm: 'Term',
    watchColSessions: 'Conversations',
    watchColComplaints: 'of which complaints',
    watchColOther: 'not complaints',
    watchUnused: 'did not appear in this range',
    watchColUnclassified: 'not yet classified',
    watchNotYetKnown: 'not yet classified',
    watchPendingHint: 'Some conversations have not been classified yet, so the complaint split is partial. "Not yet classified" is not "no complaints".',
    sectionKeywords: 'Keywords the system found',
    sectionTable: 'Conversation detail',
    colDate: 'Date',
    colChannel: 'Channel',
    colInquiryType: 'Inquiry type',
    colTopic: 'Topic',
    colComplaint: 'Complaint',
    colSummary: 'Summary',
    export: 'Export',
    exportAggregates: 'Export aggregates',
    exportWithMessages: 'Export including conversation content',
    exportWithMessagesWarning: 'This file will contain customer inquiry content. Continue?',
    filterAll: 'All',
    filterComplaintsOnly: 'Complaints only',
    filterClear: 'Clear filters',
    loading: 'Loading…',
    empty: 'No data in the selected range',
    prev: 'Previous',
    next: 'Next',
    yes: 'Yes',
    no: 'No',
  },
};
